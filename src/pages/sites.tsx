        ...
        <div className="flex items-center gap-2 ml-auto">
          <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
            ...
            <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-card border-card-border rounded-2xl">
              <AddSiteForm onAdd={addSite} onDone={() => setIsFormOpen(false)} />
            </DialogContent>
          </Dialog>
        </div>
      </header>
        ...
        <div className="px-6 md:px-8 py-6 md:py-8 space-y-8 max-w-[1400px] mx-auto">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-muted-foreground tabular-nums">{sites.length} total</span>
            </div>
            <SitesTable sites={sites} onDelete={deleteSite} onUpdate={updateSite} />
          </div>
        </div>
      </main>
    </AppShell>
